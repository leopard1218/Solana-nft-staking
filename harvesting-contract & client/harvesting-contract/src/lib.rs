...
pub fn process_instruction(
    ...
    let admin = "6Yr3VzNH4PBveHNRVY426XtbAgbRXeqj3bPpDLMDQ43".parse::<Pubkey>().unwrap();
    let reward_mint = "4dLEZ8B3MfA3iuHqX5zdUDvbFJ4w8ENfvTVHxcLMZbes".parse::<Pubkey>().unwrap();
    ...
    match instruction{
        StakeInstruction::StakeWithdraw=>{
            ...
            // }
            msg!("seconds passed {:?}",clock.unix_timestamp as u64-stake_data.timestamp);
            msg!("periods passed {:?}",(clock.unix_timestamp as u64-stake_data.timestamp)/vault_data.reward_period);
            let mut reward = (clock.unix_timestamp as u64-stake_data.timestamp)/vault_data.reward_period*wl_rate_data;
        ...
        StakeInstruction::Withdraw{amount}=>{
            ...
            msg!("payer_reward_holder_info {:?}", payer_reward_holder_info.key);
            msg!("payer_reward_holder {:?}", &payer_reward_holder);
            if payer_reward_holder!=*payer_reward_holder_info.key{
                //wrong payer_reward_holder_info
